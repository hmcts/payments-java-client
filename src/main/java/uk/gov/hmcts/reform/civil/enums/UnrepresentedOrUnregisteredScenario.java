    ...
    public static List<String> getDefendantNames(UnrepresentedOrUnregisteredScenario scenario, CaseData caseData) {
        ...
        switch (scenario) {
            case UNREPRESENTED:
                ...
                if (caseData.getRespondent2Represented() != YesOrNo.YES && caseData.getRespondent2() != null) {
                    defendantNames.add(caseData.getRespondent2().getPartyName());
                }
                break;
            case UNREGISTERED:
                if (caseData.getRespondent1OrgRegistered() != YesOrNo.YES
                        && caseData.getRespondent1OrganisationPolicy() == null
                        && caseData.getRespondent1Represented() == YesOrNo.YES) {
                    defendantNames.add(caseData.getRespondent1().getPartyName());
                }
                if (caseData.getRespondent2OrgRegistered() != YesOrNo.YES
