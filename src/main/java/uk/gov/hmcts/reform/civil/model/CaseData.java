...
import uk.gov.hmcts.reform.civil.notify.service.DeadlinesCalculator;
...
//    }
    ...
    public String setUpJudgementFormattedPermittedDate() {
        if (isJudgementDateNotPermitted()) {
            return DateFormatHelper.formatLocalDateTime(getRespondent1ResponseDate()
                    .toLocalDate().plusDays(5).atTime(DeadlinesCalculator.END_OF_BUSINESS_DAY), DateFormatHelper.DATE_TIME_AT);
        }
        return null;
    }
    ...
    public boolean isPartAdmitClaimSpec() {
        return RespondentResponseTypeSpec.PART_ADMISSION.equals(getRespondent1ClaimResponseTypeForSpec());
    }
